module multi_bit_morse_encoder(
    ...
    logic [31:0] btn1_count;
    logic [31:0] btn2_count;
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            // Button 1 debounce
            if (btn1_sync1 == btn1_db)
                btn1_count <= 0;
            else begin
                if (btn1_count == DEBOUNCE_COUNT) begin
                    btn1_db    <= btn1_sync1;
                    btn1_count <= 0;
                end
                else
            ...
            // Button 2 debounce
            if (btn2_sync1 == btn2_db)
                btn2_count <= 0;
            else begin
                if (btn2_count == DEBOUNCE_COUNT) begin
                    btn2_db    <= btn2_sync1;
                    btn2_count <= 0;
                end
                else
    ...
    // Morse Code mapping 0 -> dot and 1 -> dash
    always @(*) begin
            ...
            case (state)
                ...
                IDLE: begin
                    ...
                    if (mode == 1'b0) begin
                        ...
                        if (btn1_db && !btn1_prev && wr_index < 6) begin //btn1 && !btn1_prev makes sure data is not latched if button is kept pressed
                        ...
                        else if (btn2_db && !btn2_prev && wr_index != 0) begin //same thing for reading
                            rd_index <= 0;
                            mcount   <= 0;
                            state    <= ON;
                        end
